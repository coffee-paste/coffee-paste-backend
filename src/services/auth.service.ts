...
class AuthService {
	...
	private async validateGoogleOAuth2Session(oauth2Session: OAuth2Session): Promise<OAuthUserInfo> {
		...
		const authResponse = await fetch(
			`https://oauth2.googleapis.com/token?code=${oauth2Session.code}&client_id=${GOOGLE_CLIENT_ID}&client_secret=${GOOGLE_SECRET}&redirect_uri=${oauth2Session.redirectUri}&grant_type=authorization_code`,
			getTokenOption
		);
	...
	private async validateGitLabOAuth2Session(oauth2Session: OAuth2Session): Promise<OAuthUserInfo> {
		...
		const authResponse = await fetch(
			`https://gitlab.com/oauth/token?client_id=${GITLAB_CLIENT_ID}&client_secret=${GITLAB_SECRET}&code=${oauth2Session.code}&grant_type=authorization_code&redirect_uri=${oauth2Session.redirectUri}`,
			getTokenOption
		);
	...
	public async authByOAuth(oauth2Session: OAuth2Session): Promise<string> {
		...
		try {
			...
			logger.info(
				`[AuthService.authByOAuth] Setting user "${userId}" in system for code "${oauth2Session.code}" uniqueOAuthId "${userInfo.uniqueOAuthId}" succeed, login proses successfully done`
			);
			return jwtToken;
		} catch (error) {
			logger.error(
				`[AuthService.authByOAuth] Validate and get token / info for code "${oauth2Session.code}" using "${oauth2Session.oauth2Service}" failed`,
				error
			);
			// Throw object with status number, as TSOA does, so it will be handled the same
