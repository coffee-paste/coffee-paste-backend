...
const whitelist = ALLOW_DASHBOARD_ORIGINS ? ALLOW_DASHBOARD_ORIGINS.split(',') : ['http://127.0.0.1:8080', 'http://localhost:8080'];
...
app.get('/privacy-policy.txt', (_req, res) => {
	res.setHeader('Content-type', 'application/octet-stream');
	res.setHeader('Content-disposition', 'attachment; filename=security.txt');
	res.send(`
...
app.get('/therm-of-use.txt', (_req, res) => {
	res.setHeader('Content-type', 'application/octet-stream');
	res.setHeader('Content-disposition', 'attachment; filename=security.txt');
	res.send(`
            This service built as an open-source project at https://github.com/coffee-paste/coffee-paste-backend.
...
app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
	...
	try {
		logger.error(
			`express route crash,  req: ${req.method} ${req.path} error: ${err.message} user: ${req.user?.userId || 'unknown'} body: ${JSON.stringify(req.body)}`
		);
	} catch (error) {
